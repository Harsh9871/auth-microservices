...
class AuthService {
  async register(name, email, password, app_id, role = 'user') {
    try {
      ...
      const userData = {
        ...
        isVerified: false // Add this field for email verification
      };
      ...
      try {
        ...
        if (otpResult.success) {
          ...
          console.log('⚠️ User registered but OTP creation failed');
        }
      } catch (emailError) {
      ...
      return {
        ...
        user: {
          ...user,
          // Don't include isVerified in response if it's false
  ...
  async validateToken(token) {
    ...
    try {
      ...
      const user = await prisma.user.findUnique({
        where: { id: decoded.id },
        select: { id: true, name: true, email: true, app_id: true, role: true, isVerified: true },
      });
